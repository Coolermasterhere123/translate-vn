...
function drawFittedText(
  ...
  let fontSize = Math.min(11, Math.max(7, h * 0.6));
...
export default function TranslateVN() {
  ...
  const overlayRef   = useRef<HTMLDivElement>(null); // separate layer for buttons
  const flashRef     = useRef<HTMLDivElement>(null);
  const snapshotRef  = useRef<HTMLCanvasElement | null>(null);
  const streamRef    = useRef<MediaStream | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Zoom state — applied to a wrapper div, not the whole page
  ...
  // ── Apply zoom transform to canvas wrapper only ──────────────────────────
  const applyTransform = useCallback(() => {
    const wrap = arWrapRef.current;
    if (!wrap) return;
    ...
    wrap.style.transform = `translate(${zoomRef.current.panX}px, ${zoomRef.current.panY}px) scale(${scale})`;
  ...
  const resumeCamera = useCallback(() => {
    const wrap = arWrapRef.current;
    if (wrap) { wrap.style.transform = ''; wrap.style.display = 'none'; }
  ...
  const renderAR = useCallback((items: TranslationItem[], errMsg: string | null) => {
    ...
    // object-fit: cover math — match exactly how video renders
    const imgAspect    = snap.width / snap.height;
    const screenAspect = vw / vh;
    let drawW: number, drawH: number, offX: number, offY: number;

    if (imgAspect > screenAspect) {
      // image wider than screen — fit height, crop sides
      drawH = vh;
      drawW = vh * imgAspect;
      offX  = (vw - drawW) / 2;
      offY  = 0;
    } else {
      // image taller than screen — fit width, crop top/bottom
      drawW = vw;
      drawH = vw / imgAspect;
      offX  = 0;
      offY  = (vh - drawH) / 2;
    ...
    items.forEach(item => {
      ...
      // Map % -> canvas pixels using same cover offsets
      ...
      // Keep box on screen
      const cx = Math.max(0, Math.min(bx, vw - 20));
      const cw = Math.min(bw, vw - cx);

      // Slim height — just enough for one text line
      const textH = Math.max(16, Math.min(bh * 0.5, 22));
      // Position box at top of the bounding box (where Vietnamese text usually is)
      const textY = Math.max(0, Math.min(by, vh - textH - 2));

      ctx.fillStyle = 'rgba(0,0,0,0.88)';
      ctx.fillRect(cx, textY, cw, textH);

      ctx.fillStyle = gold;
      ctx.fillRect(cx, textY, 2, textH);

      drawFittedText(ctx, full, cx + 2, textY, cw - 2, textH);
    });
    ...
    if (errMsg || items.length === 0) {
      ...
      ctx.beginPath();
      ctx.roundRect(vw / 2 - 150, vh / 2 - 25, 300, 50, 12);
      ctx.fill();
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (e.touches.length === 2) {
      ...
      zoomRef.current.scale    = Math.min(8, Math.max(1, zoomRef.current.scale * (dist / zoomRef.current.lastDist)));
      zoomRef.current.lastDist = dist;
      applyTransform();
    } else if (e.touches.length === 1 && !zoomRef.current.isPinch && zoomRef.current.scale > 1.01) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000', overflow: 'hidden' }}>
      ...
      {/* AR canvas wrapper — this is what gets zoomed/panned */}
      <div
        ref={arWrapRef}
        style={{ position: 'absolute', inset: 0, zIndex: 2, display: 'none', touchAction: 'none' }}
      ...
      {/* ── TOP BAR — lives OUTSIDE zoom wrapper, always visible ── */}
      <div style={{
        ...
        {arActive && (
          <div style={{ background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(10px)', border: '1px solid rgba(200,146,42,0.3)', borderRadius: 20, padding: '6px 14px', fontSize: 11, color: 'rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: '#e8b84b', fontWeight: 700 }}>EN</span> Pinch zoom · Tap dismiss
          </div>
        )}
      </div>
      ...
      {/* Viewfinder */}
      {!arActive && (
        <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-58%)', width: '70vw', maxWidth: 280, aspectRatio: '1', zIndex: 8, pointerEvents: 'none', opacity: 0.6 }}>
          {(['tl', 'tr', 'bl', 'br'] as const).map(pos => {
            ...
            return <div key={pos} style={{ position: 'absolute', width: 22, height: 22, borderColor: '#e8b84b', borderStyle: 'solid', ...s[pos] }} />;
          })}
        </div>
      )}

      {/* ── BOTTOM BAR — lives OUTSIDE zoom wrapper, always visible ── */}
      <div style={{
        ...
        {/* Shutter */}
        <div onClick={arActive ? resumeCamera : capture} style={{
          ...
          boxShadow: scanning ? '0 0 30px rgba(232,184,75,0.5)' : '0 0 20px rgba(200,146,42,0.3)',
      ...
      {scanning && (
        <div style={{
          ...
          zIndex: 55, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(14px)',
          border: '1px solid rgba(200,146,42,0.3)', borderRadius: 20,
      ...
      {/* No camera */}
      {noCamera && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14, textAlign: 'center', padding: 40, background: 'radial-gradient(ellipse at center,#111,#000)' }}>
          <div style={{ fontSize: 52 }}>📷</div>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', lineHeight: 1.7 }}>Camera access is needed.<br />Please allow permission and try again.</p>
          <button onClick={startCamera} style={{ padding: '13px 28px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#c8922a,#8b5e10)', color: '#000', fontSize: 14, fontWeight: 700, cursor: 'pointer', marginTop: 10 }}>
            Enable Camera
          </button>
        </div>
      )}
